const compileUtil = {
  ...
  html(node, expr, vm) {
    ...
    // 绑定观察者, 将来数据发生变化, 触发这里的回调, 进行更新
    new Watcher(vm, expr, (newVla) => {
      // 数据更改, 重新编译更新view
      this.updater.htmlUpdater(node, newVla);
    });
  ...
  model(node, expr, vm) {
    ...
    new Watcher(vm, expr, (newVla) => {
      // 数据更改, 重新编译更新view
      this.updater.modelUpdater(node, newVla);
    });
...
实例创建的时候new Observer(this.$data)
什么时候关联Dep和添加Wacher呢?

Watcher在初始化数据, 渲染的时候 去new Watcher() 把对应的Watcher绑定到 Dep.target静态属性上

劫持监听所有属性的时候 创建Dep, 然后添加Watcher(通过Dep.target静态属性获取)

一切安好,
外部修改数据的时候  Observer 通知变化 Dep, Dep 通知变化 Watcher, Watcher 更新视图 Updater
